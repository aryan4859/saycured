function load_gf_template(pattern_name) {
  ...
  $.getJSON(
    `/api/getFileContents?gf_pattern&name=${pattern_name}&format=json`,
    function (response) {
      swal.close();
      if (response.status) {
        $("#modal_title").empty();
        ...
        $("#modal-content").append(
          `<pre>${htmlEncode(response["content"])}</pre>`
        );
...
function load_nuclei_template(pattern_name) {
  ...
  $.getJSON(
    `/api/getFileContents?nuclei_template&name=${pattern_name}&format=json`,
    function (response) {
      swal.close();
      if (response.status) {
        $("#modal_title").empty();
...
$.getJSON(`/api/getFileContents?nuclei_config&format=json`, function (data) {
  ...
  $("textarea#nuclei_config_text_area").html(
    `# Your nuclei configuration here.`
  );
  $("#nuclei-config-form").append(
    '<input type="submit" class="btn btn-primary mt-2 float-end" value="Save Changes" id="nuclei-config-submit">'
  );
});
...
$.getJSON(`/api/getFileContents?subfinder_config&format=json`, function (data) {
  ...
  $("textarea#subfinder_config_text_area").html(
    `# Your Subfinder configuration here.`
  );
  $("#subfinder-config-form").append(
    '<input type="submit" class="btn btn-primary mt-2 float-end" value="Save Changes" id="subfinder-config-submit">'
  );
});
...
$.getJSON(`/api/getFileContents?naabu_config&format=json`, function (data) {
  ...
  $("#naabu-config-form").append(
    '<input type="submit" class="btn btn-primary mt-2 float-end" value="Save Changes" id="naabu-config-submit">'
  );
});
...
$.getJSON(`/api/getFileContents?amass_config&format=json`, function (data) {
  ...
  $("#amass-config-form").append(
    '<input type="submit" class="btn btn-primary mt-2 float-end" value="Save Changes" id="amass-config-submit">'
  );
});
...
$.getJSON(
  `/api/getFileContents?theharvester_config&format=json`,
  function (data) {
    $("#theharvester_config_text_area").attr("rows", 14);
    $("textarea#theharvester_config_text_area").html(
  ...
  $("textarea#theharvester_config_text_area").html(
    `# Your the Harvester configuration here.`
  );
  $("#theHarvester-config-form").append(
    '<input type="submit" class="btn btn-primary mt-2 float-right" value="Save Changes" id="theharvester-config-submit">'
  );
});
...
$("#theharvester_config_text_area").dblclick(function () {
  if (!document.getElementById("theharvester-config-submit")) {
    $("#theharvester_config_text_area").removeAttr("readonly");
    $("#theharvester-config-form").append(
      '<input type="submit" class="btn btn-primary mt-2 float-end" value="Save Changes" id="theharvester-config-submit">'
    );
  }
});
