...
function check_saycured_update() {
  if (
    ...
    window.open("https://github.com/yogeshojha/saycured/releases", "_blank");
  } else {
    Swal.fire({
      title: "Checking saycured latest version...",
      ...
      .then(function (response) {
        ...
        if (response["description"] == "RateLimited") {
          ...
          Swal.fire({
            ...
            text: "You are running the latest version of saycured!",
            icon: "info",
          });
        }
      });
...
function update_available(latest_version_number, changelog) {
  ...
  Promise.all([
    ...
    Swal.fire({
      ...
      html: `
          ${changelogStyle}
          <h5>saycured's new update ${latest_version_number} is available, please follow the update instructions.</h5>
      ...
      if (result.isConfirmed) {
        window.open("https://www.saycured.wiki/update", "_blank");
      }
    });
  });
}
...
function showAfterUpdatePopup() {
  ...
  const currentVersion = document.body.getAttribute("data-saycured-version");
  ...
  if (lastShownVersion !== currentVersion) {
    ...
    // Swal.fire({
    //   title: isFirstRun ? "Welcome to saycured!" : "Thanks for updating!",
    ...
    // }).then((result) => {
    //   if (result.isConfirmed) {
    //     window.open("https://saycured.wiki/changelog/latest", "_blank");
    //   }
    //   localStorage.setItem("lastShownUpdateVersion", currentVersion);
    // });
    Swal.fire({
      title: "Thanks for using saycured!",
      ...
      if (result.isConfirmed) {
        window.open(
          `https://saycured.wiki/whats-new/${currentVersion.replace(
            /\./g,
            "_"
          )}`,
          "_blank"
