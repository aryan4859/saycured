  ...
  var interesting_subdomain_table = $("#interesting_subdomains").DataTable({
    drawCallback: function (settings, start, end, max, total, pre) {
      ...
      if (this.fnSettings().fnRecordsTotal() == 0) {
        ...
        // <div class="card-body">
        // saycuRed could not identify any interesting subdomains. You can customize interesting subdomain keywords <a href="/scanEngine/interesting/lookup/">from here</a> and this section would be automatically updated.
        // </div>
        // </div>`);
      } else {
        // show nav bar
    ...
    .then(function (response) {
      ...
      if (response.status) {
        ...
        $("#most_vulnerable_target_div").append(`
				...
				Once the vulnerability scan is performed, saycuRed will identify the most vulnerable targets.</div>
			`);
      }
    });
}
    ...
    .then(function (response) {
      ...
      if (response.status) {
        ...
        $("#most_common_vuln_div").append(`
				...
				Once the vulnerability scan is performed, saycuRed will identify the Most Common Vulnerabilities.</div>
			`);
      }
    });
}
...
function render_vuln_offcanvas(vuln) {
  ...
  title_content += `<span class="text-${default_color} ms-1">${htmlEncode(
    vuln.name
  )}</span>`;
  ...
  if (vuln.source == "nuclei") {
    body += `<div class="accordion custom-accordion mt-2">
		...
		<td><a target="_blank" href="${htmlEncode(vuln.template_url)}">${htmlEncode(
      vuln.template_url
    )}</a></td>
		</tr>
		<tr>
