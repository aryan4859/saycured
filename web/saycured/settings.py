...
# Root env vars
SAYCURED_HOME = env('SAYCURED_HOME', default='/usr/src/app')
SAYCURED_RESULTS = env('SAYCURED_RESULTS', default=f'{SAYCURED_HOME}/scan_results')
SAYCURED_CACHE_ENABLED = env.bool('SAYCURED_CACHE_ENABLED', default=False)
SAYCURED_RECORD_ENABLED = env.bool('SAYCURED_RECORD_ENABLED', default=True)
SAYCURED_RAISE_ON_ERROR = env.bool('SAYCURED_RAISE_ON_ERROR', default=False)

# Common env vars
DEBUG = env.bool('DEBUG', default=False)
DOMAIN_NAME = env('DOMAIN_NAME', default='localhost:8000')
TEMPLATE_DEBUG = env.bool('TEMPLATE_DEBUG', default=False)
SECRET_FILE = os.path.join(SAYCURED_HOME, 'secret')
...
SAYCURED_CURRENT_VERSION = _version
...
MIDDLEWARE = [
    ...
    'saycuRed.middleware.UserPreferencesMiddleware',
]
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'context_processors': [
                ...
                'saycuRed.context_processors.projects',
                'saycuRed.context_processors.version_context',
                'saycuRed.context_processors.user_preferences',
            ],
    },
}]
ROOT_URLCONF = 'saycuRed.urls'
REST_FRAMEWORK = {
...
WSGI_APPLICATION = 'saycuRed.wsgi.application'
...
'''
ROLEPERMISSIONS_MODULE = 'saycuRed.roles'
ROLEPERMISSIONS_REDIRECT_TO_LOGIN = True

'''
Cache settings
'''
saycuRed_TASK_IGNORE_CACHE_KWARGS = ['ctx']
...
LOGGING = {
    ...
    'formatters': {
        ...
        'task': {
            '()': lambda : SaycuredTaskFormatter('%(task_name)-34s | %(levelname)s | %(message)s')
        },
        'simple': {
