...
class SaycuRedTask(Task):
	...
	SAYCURED_RECORD_ENABLED:
	- Create / update ScanActivity object to track statuses.
	- Send notifications before and after each task (start / end).
	- Send traceback file to saycuRed's Discord channel if an exception happened.

	SAYCURED_CACHE_ENABLED:
	...
	def __call__(self, *args, **kwargs):
		...
		self.results_dir = ctx.get('results_dir', SAYCURED_RESULTS)
		...
		if SAYCURED_CACHE_ENABLED:
			...
			if result and result != b'null':
				...
				if SAYCURED_RECORD_ENABLED and self.track:
		...
		if SAYCURED_CACHE_ENABLED and self.status == SUCCESS_TASK and result:
