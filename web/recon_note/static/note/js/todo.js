        ...
        preConfirm: function () {
          return fetch("../delete_note", {
            ...
            .then(function (response) {
              if (
                !$(main_this).parents(".todo-item").hasClass("todo-task-trash")
              ) {
                ...
                if (
                  getSecondClass === "todo-task-done" ||
                  getSecondClass === "todo-task-important"
                ) {
                  getTodoParent.removeClass(getSecondClass);
                }
                if (
                  getThirdClass === "todo-task-done" ||
                  getThirdClass === "todo-task-important"
                ) {
...
function todoItem() {
  $(".todo-item .todo-content").on("click", function (event) {
    ...
    $(".task-text").html(
      `<span class="text-success">${$_taskTarget}</span><br>` +
        htmlEncode($todoDescription)
    );
...
function populateScanHistory(project) {
  ...
  $.getJSON(
    `/api/listScanHistory/?format=json&project=${project}`,
    function (data) {
      for (var history in data) {
        ...
        option.innerHTML =
          history_object["domain"]["name"] +
          " - Scanned " +
          moment.utc(history_object["start_scan_date"]).fromNow();
